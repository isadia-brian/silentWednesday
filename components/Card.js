...
const Card = ({ image, description, title }) => {
  return (
    <div className="group h-56 md:h-80 overflow-hidden cursor-pointer relative hover:shadow-xl hover:shadow-black/50 transition-shadow">
      ...
      <div className="absolute inset-0 flex flex-col px-3 max-w-[500px] translate-y-[80%] group-hover:translate-y-[20%] md:group-hover:translate-y-[80%] transition-all">
        <h1
        ...
        <p
          className={`${poppins.className} text-white font-light text-[12px] opacity-0 group-hover:opacity-100 duration-300 mt-2`}
        >
          {description}
        </p>
      </div>
